...
func ready_paint_pot():
	if !pot_made:
		...
		pot_made = true
	pot_to_instantiate.linear_velocity = paint_can_speed * get_forward_direction()
	pot_to_instantiate.global_position = global_position
	#pot_to_instantiate.thrown_direction = (get_global_mouse_position() - global_position).normalized()
...
func update_trajectory() -> void:
	var velocity : Vector2 = paint_can_speed * get_forward_direction()
